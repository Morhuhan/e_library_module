...
const AddUdkDialog: React.FC<AddUdkDialogProps> = ({ open, onClose, onPick }) => {
  ...
  useEffect(() => {
    ...
    const tId = setTimeout(async () => {
      ...
      setError(null);
      try {
        const { data } = await httpClient.get('/udc', {
          params: { q: query.trim() },
          signal: ctrl.signal,
        });
        ...
        if (err.name !== 'CanceledError') {
          console.error('Ошибка при поиске УДК:', err);
